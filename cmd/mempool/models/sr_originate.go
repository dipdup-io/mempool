...
type SmartRollupOriginate struct {
	...
	Source           string `json:"source,omitempty" index:"sr_originate_source_idx" comment:"Address of the account who has sent the operation."` // DISCUSS
	PvmKind          string `json:"pvm_kind" comment:"PVM kind (arith or wasm)."`
	Kernel           string `json:"kernel" comment:"Kernel bytes (in base64)."`
	OriginationProof string `json:"origination_proof" comment:"Origination proof bytes (in base64)."`
	ParametersTy     string `json:"parameters_ty" comment:"Smart rollup parameter type."`
}
